...
- **`{% liquid %}` is line-based — a conditional's operators can't wrap onto their own line.** `if a\n  or b\n  or c` fails with `Unknown tag 'or'` because each line inside a `{% liquid %}` block is parsed as its own statement. Keep the whole `if`/`unless`/`case` expression on one line, however long.
- **`assign` cannot evaluate a comparison expression** — `assign has_thing = my_value != blank` is invalid Liquid, only `if`/`unless`/`case` accept boolean expressions. Pattern: `assign has_thing = false` then `unless my_value == blank / assign has_thing = true / endunless`.
- **`false == blank` is `true` in Liquid.** A boolean-typed value (e.g. a `true_false` metafield's `.value`) that's actually set to `false` will incorrectly read as "unset" if you guard on `!= blank`. Check the wrapping object instead — e.g. `if product.metafields.ns.my_bool != blank` (the metafield resource, not `.value`) — to distinguish "not set" from "set to false."
- **Liquid's math filters (`times`, `plus`, `minus`, `divided_by`, etc.) convert strings via a strict `BigDecimal` parse, not a lenient `to_f`.** `"30g" | times: 1.0` doesn't parse `30` and ignore the `g` — the conversion raises internally and Liquid silently rescues it to `0`. No error surfaces anywhere; the output is just silently wrong. If a value might carry a non-numeric suffix (units, etc.), strip it first (`| remove: 'g' | strip`) before doing math on it.
- **`shopify theme check` does not catch every Liquid syntax error** — both of the `{% liquid %}` issues above passed `theme check` cleanly and only surfaced as a hard 500 from the actual Shopify Liquid parser on `shopify theme dev`/live storefront. Treat a clean `theme check` run as necessary, not sufficient, for anything using `{% liquid %}` multi-statement blocks; sanity-check non-trivial ones against a running dev server.
- **CSS custom properties feeding `rgb(var(--x) / alpha)` need a space-separated triplet** (`57 153 153`), not comma-separated (`57, 153, 153`) — mixing legacy comma notation with the modern slash-alpha syntax makes the whole declaration invalid, and the browser silently drops it (falls back to whatever rule matched before it) rather than erroring visibly. This bites you specifically when piping Shopify's `<color>.rgb` Liquid filter — which itself outputs comma-separated — into that pattern; reformat with `| replace: ',', ' '` first.
- **Never let new block content share a wrapper with a `position: sticky` element** (e.g. `<media-gallery>` inside `.product-information__media`, which is unconditionally `sticky-content`). Nesting content below the sticky element inside the same container inflates that container's height, which gives the sticky element room to stay pinned and visually paint over the new content while the page scrolls. Render new content as a sibling (e.g. its own subgrid row placed next to, not inside, the sticky wrapper) instead.
...
- `shopify theme dev` — starts a local dev server against a connected store to preview changes. It doesn't always hot-register a **brand-new** `blocks/`/`sections/` file added while it's already running (edits to existing files sync fine) — if a template references a new block type and the dev server serves an "Invalid value for type... must be defined in the theme blocks folder" error even though the file exists and `theme check` is clean, restart the dev server before debugging further.
